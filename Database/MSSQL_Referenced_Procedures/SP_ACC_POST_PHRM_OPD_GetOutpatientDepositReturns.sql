CREATE PROCEDURE [dbo].[SP_ACC_POST_PHRM_OPD_GetOutpatientDepositReturns]
	@TransactionDate DATE
	,@HospitalId INT
AS
/*
 exec SP_ACC_POST_PHRM_OPD_GetOutpatientDepositReturns '2023-06-19',1

 Change History
SN.                Auther/DateTime                   Description
1.                 DevN/19th June 23                Initial Draft of SP to get OPD Pharmacy Deposit(DepositDeduct) Transactions.
*/
BEGIN
SELECT 
	TransactionType
...
FROM (
	SELECT 
		...
		,'PHRM_OPD_DepositAdjustment' AS TransactionType
		,deposit.OutAmount AS TotalAmount
		,CAST(deposit.CreatedOn AS DATE) AS TransactionDate
		,'OPD Deposit Adjusted for ' + CONVERT(VARCHAR(100), CAST(deposit.CreatedOn AS DATE)) AS Description
