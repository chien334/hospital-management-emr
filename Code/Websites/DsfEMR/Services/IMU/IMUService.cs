    ...
    public class IMUService : IIMUService
    {
        ...
        public Task<IMUResponseModel> PostDataToIMU(LabDbContext labDbContex, CoreDbContext coreDbContext, List<Int64> reqIdList, RbacUser currentUser)
        {
            ...
            reqIdList.ForEach(async a =>
            {
                ...
                var userName = CommonFunctions.GetCoreParameterValueByKeyName_String(coreDbContext, "LAB-IMU", "DsfConfigForIMU", "UserName");
                var password = CommonFunctions.GetCoreParameterValueByKeyName_String(coreDbContext, "LAB-IMU", "DsfConfigForIMU", "Password");
                var url = CommonFunctions.GetCoreParameterValueByKeyName_String(coreDbContext, "LAB-IMU", "DsfConfigForIMU", "FullUrl"); ;
        ...
        public Task<IMUResponseModel> PostToIMU(IMUPostDataModel IMUmodel, string url, string userName, string password)
        {
            ...
            {
                var jsonData = DsfJSONConvert.SerializeObject(IMUmodel);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    res = DsfJSONConvert.DeserializeObject<IMUResponseModel>(message.Result);                  
                }
                else
                {
