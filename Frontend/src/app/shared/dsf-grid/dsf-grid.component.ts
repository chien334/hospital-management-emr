...
@Component({
  selector: "dsf-grid",
  templateUrl: "./dsf-grid.html",
})
export class DsfGridComponent implements OnInit, AfterViewInit {
  ...
  // NOTE: each custom item(eg, image, button, anchor, should have this attribute "dsf-grid-action" in its template, else it won't work..
  public onCellClicked($event) {
    if ($event.event.target !== undefined) {
      let data = $event.data;
      let actionType = $event.event.target.getAttribute("dsf-grid-action");
  ...
  public ExportToExcel($event) {
    ...
    if (this.showNepaliDateOption) {
      this.nepaliDateInGridColDetail.NepaliDateColumnList.forEach((col) => {
        ...
        let fld = this.columnDefs[ind].field;
        DsfGridComponent.colIdObj[fld] = true;
        DsfGridComponent.colIdObj[fld + "showTime"] = col.ShowTime;
      });
    }
    ...
    function cellFormattingFunction(params) {
      ...
      if (DsfGridComponent.colIdObj[colId]) {
        if (DsfGridComponent.colIdObj[colId + "showTime"]) {
