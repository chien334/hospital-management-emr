...
export class VisitDLService {
  ...
  public getMemberInfoByScheme(schemeId: number, patientId: number) {
    return this.http.get<DsfHTTPResponse>(`/api/Visit/GetMemberInformationByScheme?schemeId=${schemeId}&patientId=${patientId}`);
  }
  public GetPatientCreditLimitsByScheme(schemeId: number, patientId: number, serviceBillingContext: string) {
    return this.http.get<DsfHTTPResponse>(`/api/Visit/GetPatientCreditLimitsByScheme?schemeId=${schemeId}&patientId=${patientId}&serviceBillingContext=${serviceBillingContext}`);
  }
  public GetLatestClaimCodeForAutoGeneratedClaimCodes(schemeId: number) {
    return this.http.get<DsfHTTPResponse>(`/api/Visit/GetLatestClaimCode?schemeId=${schemeId}`);
  }
}
