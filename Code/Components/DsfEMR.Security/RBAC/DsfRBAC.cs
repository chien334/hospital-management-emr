...
namespace DsfEMR.Security
{
    ...
    public class RBAC
    {
        ...
        {
            List<RbacApplication> retList = (List<RbacApplication>)DsfCache.Get("RBAC-Apps-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.Applications.ToList();
                DsfCache.Add("RBAC-Apps-All", retList, cacheExpiryMinutes);
            }
            return retList;
        }
        ...
        {
            List<RbacPermission> retList = (List<RbacPermission>)DsfCache.Get("RBAC-Perms-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.Permissions.ToList();
                DsfCache.Add("RBAC-Perms-All", retList, cacheExpiryMinutes);
            }
            return retList;
        ...
        {
            List<RbacRole> retList = (List<RbacRole>)DsfCache.Get("RBAC-Roles-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.Roles.ToList();
                DsfCache.Add("RBAC-Roles-All", retList, cacheExpiryMinutes);
            }
            return retList;
        }
        ...
        {
            List<RbacUser> retList = (List<RbacUser>)DsfCache.Get("RBAC-Users-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.Users.ToList();
                DsfCache.Add("RBAC-Users-All", retList, cacheExpiryMinutes);
            }
            return retList;
        }
        ...
        {
            List<UserRoleMap> retList = (List<UserRoleMap>)DsfCache.Get("RBAC-UserRoleMaps-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.UserRoleMaps.ToList();
                DsfCache.Add("RBAC-UserRoleMaps-All", retList, cacheExpiryMinutes);
            }
        ...
        {
            List<RolePermissionMap> retList = (List<RolePermissionMap>)DsfCache.Get("RBAC-RolePermissionMaps-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.RolePermissionMaps.ToList();
                DsfCache.Add("RBAC-RolePermissionMaps-All", retList, cacheExpiryMinutes);
            }
            return retList;
        }
        ...
        {
            List<DsfRoute> retList = (List<DsfRoute>)DsfCache.Get("RBAC-Routes-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.Routes.ToList();
                DsfCache.Add("RBAC-Routes-All", retList, cacheExpiryMinutes);
            }
            return retList;
        ...
        public static List<DsfRoute> GetRoutesForUser(int userId, bool getHiearrchy)
        {
            List<DsfRoute> allRoutes = new List<DsfRoute>();
            ...
            {
                //don't get hidden routes if it's set to false.
                List<DsfRoute> parentRoutes = allRoutes.Where(a => a.ParentRouteId == null && a.DefaultShow == true).ToList();
        ...
        //don't get hidden routes if it's set to false.
        private static List<DsfRoute> GetChildRouteHierarchy(List<DsfRoute> searchList, DsfRoute route)
        {
            //don't get hidden routes if it's set to false.
            List<DsfRoute> childRoutes = searchList.Where(a => a.ParentRouteId == route.RouteId).Distinct().ToList();
        ...
        {
            List<RbacPermission> retList = (List<RbacPermission>)DsfCache.Get("RBAC-UserPermissions-UserId" + userId);
            if (retList == null)
            {
                ...
                DsfCache.Add("RBAC-UserPermissions-UserId" + userId, retList, cacheExpiryMinutes);
            }
            return retList;
