...
import { ENUM_DsfHTTPResponses, ENUM_DsfSSFSchemes, ENUM_MessageBox_Status, ENUM_SSF_EligibilityType, ENUM_Scheme_ApiIntegrationNames, ENUM_ServiceBillingContext } from "../../../shared/shared-enums";
...
export class RegistrationSchemeSelectComponent {
  ...
  GetLatestClaimCode(schemeId: number): void {
    this.visitBlService.GetLatestClaimCodeForAutoGeneratedClaimCodes(schemeId).subscribe((res: DsfHTTPResponse) => {
  ...
  LoadMemberInformationByScheme(schemeId: number, currentPatientId: number): void {
    this.visitBlService.getMemberInfoByScheme(schemeId, currentPatientId).subscribe((res: DsfHTTPResponse) => {
      if (res.Status === ENUM_DsfHTTPResponses.OK) {
  ...
  LoadMedicarePatientInformation(patientId: number): void {
    this.visitBlService.getMedicareMemberDetail(patientId).subscribe((res: DsfHTTPResponse) => {
      if (res.Status === ENUM_DsfHTTPResponses.OK) {
      ...
      this.msgBoxService.showMessage(ENUM_DsfHTTPResponses.Failed, ["Could not fetch the Medicare Member Detail"]);
    });
  }
  ...
  AssignSsfPatientData(ssfData: SsfDataStatus_DTO) {
    ...
    if (this.currentRegSchemeDto.SchemeName.toLowerCase() === ENUM_DsfSSFSchemes.Medical.toLowerCase()) {
    ...
    if (eligibility && eligibility.length && this.currentRegSchemeDto.SchemeName.toLowerCase() === ENUM_DsfSSFSchemes.Medical.toLowerCase()) {
    ...
    } else if (eligibility && eligibility.length && this.currentRegSchemeDto.SchemeName.toLowerCase() !== ENUM_DsfSSFSchemes.Medical.toLowerCase()) {
