CREATE PROCEDURE [dbo].[SP_ACC_POST_BIL_OPD_GetOutpatientDeposits]
	@TransactionDate DATE
	,@HospitalId INT
AS
/*
 exec SP_ACC_POST_BIL_OPD_GetOutpatientDeposits '2023-06-15',1

 Change History
SN.                Auther/DateTime                   Description
1.                 DevN/15th June 23                Initial Draft of SP to get OPD Billing Deposit Transactions.
...
FROM (
	SELECT 
		...
		,'BIL_OPD_Deposit' AS TransactionType
		,deposit.InAmount AS TotalAmount
		,CAST(deposit.CreatedOn AS DATE) AS TransactionDate
		,'Deposit Collection for ' + CONVERT(VARCHAR(100), CAST(deposit.CreatedOn AS DATE)) AS Description
	...
	--AND deposit.VisitType IN ('outpatient','emergency')
