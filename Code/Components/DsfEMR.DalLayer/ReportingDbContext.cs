...
namespace DsfEMR.DalLayer
{
    public class ReportingDbContext : DbContext
    {
        ...
        public List<SqlAuditModel> SqlAuditDetails(DateTime FromDate, DateTime ToDate, string LogType)
        {
            ...
            if (isPostgres)
            {
                var data = Database.SqlQueryRaw<SqlAuditModel>("SELECT * FROM sp_dsf_sqlaudit(@FromDate,@ToDate,@LogType)",
            ...
            {
                var data = Database.SqlQueryRaw<SqlAuditModel>("exec SP_Dsf_SQLAudit @FromDate,@ToDate,@LogType",
                     new SqlParameter("@FromDate", FromDate),
                     new SqlParameter("@ToDate", ToDate),
        ...
        public List<AuditTrailModel> AuditTrailList()
        {
            ...
            if (isPostgres)
            {
                var data = Database.SqlQueryRaw<AuditTrailModel>("SELECT * FROM sp_dsf_audit_list()").ToList();
                return data;
            }
            else
            {
                var data = Database.SqlQueryRaw<AuditTrailModel>("exec SP_Dsf_Audit_List ").ToList();
        ...
        public DataTable AuditTrails(DateTime FromDate, DateTime ToDate, string Table_Name, string UserName, string ActionName)
        {
            ...
            DataTable returnAuditData = DALFunctions.GetDataTableFromStoredProc("SP_Dsf_Audit", paramList, this);
