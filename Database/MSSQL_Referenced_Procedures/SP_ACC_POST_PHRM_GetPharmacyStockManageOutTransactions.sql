CREATE PROCEDURE [dbo].[SP_ACC_POST_PHRM_GetPharmacyStockManageOutTransactions]
	@TransactionDate DATE
	,@HospitalId INT
AS
/*
 exec SP_ACC_POST_PHRM_GetPharmacyStockManageOutTransactions '2023-06-27',1

 Change History
SN.                Auther/DateTime                   Description
1.                 DevN/27th June 23                Initial Draft of SP to get pharmacy StockManageOut transactions.
*/
BEGIN
SELECT 
	TransactionType
	...
	,DrCr AS DrCr
	,'PHRM_StockManageOut' AS BaseTransactionType
...
FROM (
	SELECT 
		...
		,'PHARMACY STOCK MANAGE OUT FOR : ' + '(' + CONVERT(VARCHAR(100), CAST(stockTxn.TransactionDate AS DATE)) +')'  AS Description
		,0 AS LedgerId
		,0 AS SubLedgerId
