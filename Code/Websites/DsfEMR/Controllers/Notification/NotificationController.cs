...
namespace DsfEMR.Controllers
{
    ...
    //            //if (reqType == "mark-as-read")
    //            //{
    //            //    List<NotificationViewModel> messageList = DsfJSONConvert.DeserializeObject<List<NotificationViewModel>>(str);
    ...
    private object UpdateNotificationAsRead(string ipDataStr, int EmployeeId)
        {
            List<NotificationViewModel> messageList = DsfJSONConvert.DeserializeObject<List<NotificationViewModel>>(ipDataStr);
            if (messageList != null && messageList.Count > 0)
            {
                foreach (NotificationViewModel msg in messageList)
                {
