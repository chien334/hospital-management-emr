...
//using Microsoft.EntityFrameworkCore;
using DsfEMR.ServerModel.AppointmentModels;
using DsfEMR.Services.SSF;
using Microsoft.EntityFrameworkCore.Metadata.Internal;
using OfficeOpenXml.FormulaParsing.Excel.Functions.Math;
using Rank = DsfEMR.ServerModel.AppointmentModels.Rank;
            ...
            try
            {
                if (clientTransaction.BillingTransactionItems != null && clientTransaction.BillingTransactionItems.Count > 0)
                {
                    ...
                    if (realTimeRemoteSyncEnabled)
                    {
                        ...
                        DsfEMR.Controllers.VisitBL.SyncBillToRemoteServer(clientTransaction, "sales", visitDbContext);
                    }
                }
                else
                {
       ...
       */
        ...
        private int GetMaximumLastVisitDays()
        {
            ...
            if (parameter != null)
            {
                ...
                var data = DsfJSONConvert.DeserializeObject<int>(paramValueStr);
                if (data > 0)
                {
