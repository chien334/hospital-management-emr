 ...
 */
@Directive({
  selector: "[auto-complete], [dsf-auto-complete]"
})
export class DsfAutoCompleteDirective implements OnInit {
  ...
  componentRef: ComponentRef<DsfAutoCompleteComponent>;
  ...
  ngOnInit(): void {
    ...
    this.wrapperEl.className = "dsf-auto-complete-wrapper";
  ...
  showAutoCompleteDropdown = (event?: any): void => {
    ...
    let factory = this.resolver.resolveComponentFactory(DsfAutoCompleteComponent);
  ...
  styleAutoCompleteDropdown = () => {
    if (this.componentRef) {
      ...
      this.acDropdownEl.style.zIndex = "12";//sud:23Jul'19--change to 12 from 1 since it was coming below focused cell of dsf-grid.
  ...
  public keydownEventHandler = (evt: any) => {
    if (this.componentRef) {
      let component = <DsfAutoCompleteComponent>this.componentRef.instance;
